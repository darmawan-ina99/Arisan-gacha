...
async function loadData() {
  try {
    ...
    if (gRes.grup) {
      ...
      grupId = gRes.grup.id; // pastikan grupId selalu terupdate
    }
...
async function simpanPengaturan() {
  ...
  if (!isAdmin) {
    ...
    if (pin === null) return;
    const res = await api('verifyPin', { pin });
    if (!res.ok) return alert('❌ PIN salah!');
    isAdmin = true;
    adminPin = pin;
    localStorage.setItem('admin-session', Date.now());
  } else if (!adminPin) {
  ...
  await api('saveGrup', { nama, iuran, admin_pin: adminPin });
...
async function tambahAnggota() {
  // Pastikan grupId tersedia
  if (!grupId) {
    alert('Grup belum siap, coba refresh halaman.');
    return;
  }
  ...
  try {
    ...
    alert('Terjadi kesalahan saat menambah anggota. Periksa koneksi.');
    console.error(e);
  }
}
